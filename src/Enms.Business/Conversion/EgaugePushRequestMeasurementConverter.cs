  ...
  EgaugeMeasurementModel>
{
  ...
  public override EgaugeMeasurementModel ToMeasurement(
    string pushRequest, string meterId, DateTimeOffset timestamp)
  {
    using var stream = new MemoryStream(Encoding.UTF8.GetBytes(pushRequest));
    var result = XDocument.Load(stream) ?? throw new InvalidOperationException(
      $"Could not deserialize xml string '{pushRequest}' to type '{nameof(XDocument)}'"
    );

    return Parse(result);
  }

  protected override string ToPushRequest(EgaugeMeasurementModel measurement)
  {
  ...
  private static EgaugeMeasurementModel Parse(XDocument xml)
  {
    ...
    var measurementRegisters = new EguageRegisterMap(
      result,
      "egauge",
      timestamp
    );
